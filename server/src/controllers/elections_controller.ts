...
class ElectionsController {
    ...
    async getSurveys(req:Request, res:Response) {
        try {
            const dataHeaders = await link_db.query(`SELECT id, titulo AS title, DATE_FORMAT(fechaInicio, '%Y-%m-%d') AS dateStart,  DATE_FORMAT(fechaFin, '%Y-%m-%d') AS dateEnd, descripcion AS description FROM votaciones ORDER BY id DESC`)
            ...
            if (dataHeaders.length < 1) {
                ...
                for (let i = 0; i < dataHeaders.length; i++) {
                    ...
                    const votedSurvey = await link_db.query(`SELECT opcion FROM dep_votaciones_personas AS a WHERE persona = ${ident} AND (SELECT encuesta FROM preguntas_votacion WHERE id = a.id) = ${survey["id"]}`);
